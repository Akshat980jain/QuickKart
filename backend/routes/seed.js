export function generateMockProducts(count) {
  const categories = [
    ...
    'books', 'toys', 'automotive', 'garden', 'food'
  ];
  const adjectives = [
  ...
  function generateDiscount() {
    const shouldHaveDiscount = Math.random() < 0.3;
  ...
  for (let i = 0; i < count; i++) {
    ...
    const productType = getRandomElement(productTypes[category]);
    const name = `${adjective} ${productType}`;
    products.push({
      name,
      description: descriptions[category],
      price: generatePrice(),
      image: getRandomElement(imageUrls[category]),
      category,
